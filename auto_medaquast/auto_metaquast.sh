...
if [ "$1" = '-h' ] || [ "$1" = '-help' ] || [ "$#" = 0 ];then echo "
    ...
    -r: reference list, will just be redirected to the --reference-list parameter in metaquast. So NO HEADER!
    -s: golden standard genome. It is highly recommanded because sometimes metaquast can not get the right genome from NCBI and results in wrong evaluation. Don't need the whole name, e.g. if your file name is 'STD.fa.gz', you just need to input 'STD'.
    -d: depth file, receive a table contains the strains (1st columns) and the depth (2nd columns), NO HEADER! TSV FORMAT!
    -m: metadata file, receive a table contains the file names of genome (1st columns) and their groupby information (2nd columns), NO HEADER! TSV FORMAT! Don't need the whole name, e.g. if your file name is 'flye.fa.gz', you just need to input 'flye'.
    -o: output directory
    -n: redo the metaquast, no matter the output directory existed or not.
    "
    exit 0
fi
...
while getopts "i:r:s:d:m:o:n" arg
do
    case $arg in
    i)
        fa_dir=$OPTARG
...
f_list1=$(find $fa_dir -type f \( -name "*.fa" -o -name "*.fasta" -o -name "*.fa.gz" -o -name "*.fasta.gz" \)|grep "$gold_genome")
